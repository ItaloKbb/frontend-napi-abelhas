...
const loadOrganizacoes = async (): Promise<SelectOption[]> => {
  ...
  const data = (await response.json()) as Array<{ id: string; name: string; slug: string | null }> | { message?: string };
  if (!response.ok || !Array.isArray(data)) {
    throw new Error("message" in data && data.message ? data.message : "Erro ao carregar organizações.");
  }
...
const fields: FieldConfig[] = [
  ...
  { key: "instituicaoId", label: "Instituição", type: "searchable-select", required: true, placeholder: "Digite para pesquisar uma organização…", loadOptions: loadOrganizacoes },
